...
const Player: FC = () => {
  const { playerId, setIsPlayerIdChanged } = AppStateValue();
  ...
  const getInit = async () => {
    ...
    console.log(playerId)
      const data = await  getTrackInfo(playerId);
  ...
  const isError = data && !data.preview_url;
  ...
  const [volume, setVolume] = useState(
    Number(localStorage.getItem("bmusic-volume")) || 0.5
  );
  const [isMuted, setIsMuted] = useState(
    !!Number(localStorage.getItem("bmusic-muted")) || false
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem("bmusic-volume", String(volume));
    localStorage.setItem("bmusic-muted", String(+isMuted));
  }, [volume, isMuted]);
